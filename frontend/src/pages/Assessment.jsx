...
function AssessmentPage() {
  ...
  const handleSubmit = () => {
    const score = answers.reduce((acc, answer, index) => acc + (answer === KNOWLEDGE_QUESTIONS[index].answer ? 20 : 0), 0);
    const gap = 100 - score;
    const weakAreas = KNOWLEDGE_QUESTIONS.filter((q, index) => answers[index] !== q.answer).map((q) => q.topic);
    setReport({ score, gap, level: getKnowledgeLevel(score), weakAreas });
    setSubmitted(true);
  };
  return (
